...
public class EmployeeAttendanceLiveUpdateStream : IEmployeeAttendanceLiveUpdateSink
{
    private readonly ConcurrentDictionary<string, ConcurrentDictionary<Guid, Channel<EmployeeAttendanceLiveRowDto>>> _subscribers =
        new(StringComparer.OrdinalIgnoreCase);

    public ChannelReader<EmployeeAttendanceLiveRowDto> Subscribe(
        string campus,
        CancellationToken cancellationToken = default)
    {
        var channel = Channel.CreateUnbounded<EmployeeAttendanceLiveRowDto>(
        ...
        var campusSubscribers = _subscribers.GetOrAdd(
            normalizedCampus,
            _ => new ConcurrentDictionary<Guid, Channel<EmployeeAttendanceLiveRowDto>>());
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (!subscriber.Value.Writer.TryWrite(row))
            {
