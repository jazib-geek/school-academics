/*
  ...
  How to use for the next teacher:
    1. Change @EmpId / name guard
    2. Replace rows in #Punches
    3. Re-run guard → preview → insert → verify
*/
...
DECLARE @EmpId        INT          = 6;
DECLARE @ExpectedName NVARCHAR(50) = N'%Laiba%';
...
--    Spreadsheet "1:00" / "1:10" / "1:20" interpreted as 13:xx PM.
----------------------------------------------------------------------------
...
VALUES
    ('2026-08-05', '08:30', '13:00'),
    ('2026-08-08', '08:25', '13:20'),
    ('2026-08-13', '08:30', '13:00'),
    ('2026-08-20', '08:00', '13:10'),
    ('2026-08-21', '08:10', '12:30');

----------------------------------------------------------------------------
-- 3) Computed preview (same LC / salary math as EmployeeAttendanceCalculator)
--    Explicit CREATE TABLE avoids SSMS temp-table schema cache (Msg 207) when
--    re-running after an older #AttendanceBackfillRows definition in the same session.
----------------------------------------------------------------------------
...
(
    SELECT
        b.*,
        /* DesignationTimeHelper.CalculateLateMinutes
           Grace forgives up to CI+grace; once past that, late mins count from CI. */
        CASE
        ...
        END AS LateMinutes,
        /* EmployeeAttendanceCalculator.CalculateEarlyMinutes */
        CASE
...
(
    SELECT
        ...
        /* CountLateComings: Min((mins-1)/30 + 1, 7) */
        CASE
...
SELECT
    ...
    m.LateDeductionOut,
    m.TodaySalaryOut,
    /* BuildManualChangeJson("ManualMarkPresent", ...) — Source drives IsManuallyAdjusted in UI */
    CONCAT(
...
/* Patch prior backfill rows that were inserted without ChangeJson */
UPDATE a
SET
...
----------------------------------------------------------------------------
-- 5) Post-check: Emp August 2026 attendance
----------------------------------------------------------------------------
SELECT
