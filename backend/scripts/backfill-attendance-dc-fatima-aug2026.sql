/*
  ...
  Teacher: Miss Fatima / Fatime (EmpID = 16)
...
DECLARE @EmpId        INT          = 16;
DECLARE @ExpectedName NVARCHAR(50) = N'%Fatim%'; -- Fatima / Fatime
...
----------------------------------------------------------------------------
-- 2) Punch list (only days with CI from the sheet)
--    "1:00"/"1:45" → 13:xx; "2:00" → 14:00.
--    Day 20 has CI only (no CO). Stray "1:46" with no day ignored.
----------------------------------------------------------------------------
...
VALUES
    ('2026-08-04', '09:30', '14:00'),
    ('2026-08-05', '09:00', '13:00'),
    ('2026-08-20', '07:40', NULL),
    ('2026-08-21', '07:45', '13:00'),
    ('2026-08-22', '07:45', '13:45');
...
(
    SELECT
        ...
        END AS LateMinutes,
        CASE
            WHEN b.CheckOutTime IS NOT NULL AND b.CheckOutTime < b.ExpectedCheckOut
...
SELECT
    ...
    CASE
        WHEN m.CheckOutTime IS NULL THEN NULL
        ELSE DATEADD(DAY, DATEDIFF(DAY, 0, m.AttendanceDate), CAST(m.CheckOutTime AS datetime))
    END,
    CASE
        WHEN m.CheckOutTime IS NULL THEN NULL
        ELSE CONVERT(char(5), m.CheckOutTime, 108)
    END,
    m.ExpectedCheckIn,
    m.ExpectedCheckOut,
    m.GraceMinutes,
    ...
    CONCAT(
        ...
        N'"After":{"Time":"', CONVERT(char(5), m.CheckInTime, 108),
            N'","CheckOutTime":',
            CASE
                WHEN m.CheckOutTime IS NULL THEN N'null'
                ELSE CONCAT(N'"', CONVERT(char(5), m.CheckOutTime, 108), N'"')
            END,
