...
function AcademicSubjectsPage() {
  ...
  const [form, setForm] = useState({ id: '', subjectName: '' })

  const load = useCallback(async () => {
    setIsLoading(true)
    setError('')
    try {
      const data = await getAcademicSubjects()
      setRows(data)
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Unable to load subjects.')
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const onSubmit = async (event) => {
    ...
    setError('')
    try {
      await upsertAcademicSubject({
        id: form.id ? Number(form.id) : undefined,
        subjectName: form.subjectName,
      })
      await load()
      setIsModalOpen(false)
      setForm({ id: '', subjectName: '' })
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Subject save failed.')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const openEditModal = (item) => {
    setError('')
    setForm({ id: String(item.id), subjectName: item.subjectName })
    setIsModalOpen(true)
  }
  ...
  const onDelete = async (itemId) => {
    ...
    setError('')
    try {
      await deleteAcademicSubject(itemId)
      await load()
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Subject delete failed.')
    } finally {
      setDeletingId(null)
    }
  }
  ...
  return (
    <AcademicLayout
      pageTitle="Subjects"
      pageSubtitle="Manage shared academic subjects."
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-slate-800">
                {form.id ? 'Edit Subject' : 'Add New Subject'}
              </h3>
              <p className="mt-1 text-sm text-slate-500">Enter subject details below.</p>
            </div>
            <button
              type="button"
              onClick={closeModal}
          ...
          <form onSubmit={onSubmit} className="mt-4 space-y-3">
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((previous) => ({ ...previous, subjectName: event.target.value }))
                }
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              >
                ...
                {isSaving ? 'Saving...' : form.id ? 'Update Subject' : 'Save Subject'}
              </button>
            </div>
          </form>
        </div>
