...
import { Heading2, Loader2, Pencil, Plus, Save, Trash2, X } from 'lucide-react'
import AcademicLayout from '../../../components/academics/AcademicLayout'
import {
  deleteAcademicExamTitle,
  getAcademicExamTitles,
  upsertAcademicExamTitle,
} from '../../../services/academicExamTitleService'

function AcademicExamTitlesPage() {
  ...
  const [form, setForm] = useState({ id: '', title: '', examType: '' })

  const load = useCallback(async () => {
    setIsLoading(true)
    setError('')
    try {
      const data = await getAcademicExamTitles()
      setRows(data)
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Unable to load exam titles.')
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      setForm({ id: '', title: '', examType: '' })
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Exam title save failed.')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const onDelete = async (itemId) => {
    ...
    setError('')
    try {
      await deleteAcademicExamTitle(itemId)
      await load()
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Exam title delete failed.')
    } finally {
      setDeletingId(null)
    }
  }
  ...
  return (
    <AcademicLayout
      pageTitle="Exam titles"
      pageSubtitle="Manage standardized exam names for papers (filter later by exam type)."
      pageIcon={<Heading2 size={18} />}
      isSingleCardLayout
      pageActions={
    ...
    >
      ...
      <section className="max-h-[65vh] overflow-auto rounded-xl border border-slate-200">
        <table className="w-full min-w-[520px] border-collapse text-[13px] leading-snug md:min-w-[720px]">
          <thead className="sticky top-0 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              <th className="border border-slate-200 px-2 py-2 font-semibold md:px-3 md:py-2">Sr #</th>
              <th className="border border-slate-200 px-2 py-2 font-semibold md:px-3 md:py-2">Title</th>
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={4} className="border border-slate-200 px-2 py-12 md:px-4">
                  <div className="flex items-center justify-center gap-2 text-slate-500">
                    <Loader2 size={18} className="animate-spin text-[#405189]" />
                    <span>Loading exam titles...</span>
                  </div>
                </td>
              </tr>
            ) : rows.length === 0 ? (
              <tr>
                ...
                >
                  No exam titles yet. Add one for use in Exam Maker.
                </td>
              </tr>
            ) : (
              rows.map((item, index) => (
                <tr key={item.id} className="hover:bg-slate-50/70">
                  ...
                  <td className="border border-slate-200 px-2 py-1.5 text-slate-600 md:px-3 md:py-1.5">
                    {item.examType || '—'}
                  </td>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-slate-800">
                {form.id ? 'Edit exam title' : 'Add exam title'}
              </h3>
              <p className="mt-1 text-sm text-slate-500">
                Title appears on printed papers. Exam type key supports future filtering.
              </p>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={onSubmit} className="mt-4 space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 1st Term Exam, 2026-27"
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">
                Exam type key (optional)
              </label>
              <input
                value={form.examType}
                onChange={(event) => setForm((previous) => ({ ...previous, examType: event.target.value }))}
                placeholder="e.g. objective, subjective"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none ring-0 transition focus:border-[#405189] focus:ring-2 focus:ring-indigo-100"
              />
            </div>
            <div className="flex items-center justify-end gap-2 pt-1">
              <button type="button" onClick={closeModal} disabled={isSaving} className="btn-soft btn-soft-primary">
                Cancel
              </button>
...
export default AcademicExamTitlesPage
