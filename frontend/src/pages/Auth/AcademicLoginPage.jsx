...
import { academicLogin } from '../../services/academicAuthService'

function AcademicLoginPage() {
  const { setInstituteSettings } = useAcademicInstituteSettings()
  ...
  const onSubmit = async (event) => {
    ...
    setIsLoading(true)
    try {
      const payload = await academicLogin(form)
      setInstituteSettings(payload?.instituteSettings ?? null)
      const destination = location.state?.from?.pathname || '/academics/dashboard'
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-md items-center">
        <div className="w-full rounded-3xl bg-white/95 p-8 shadow-2xl backdrop-blur">
          <div className="mb-8 text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-xl bg-indigo-100" />
            <h1 className="text-4xl font-bold leading-tight text-indigo-900">Academics Portal</h1>
            <p className="mt-2 text-sm text-slate-500">
              Sign in to manage question papers and schedules
            </p>
          </div>
