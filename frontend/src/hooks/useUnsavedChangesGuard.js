...
/**
 * Blocks in-app link navigation and tab close/refresh when there are unsaved edits.
 * HashRouter is not a data router, so useBlocker is unavailable.
 *
 * @param {{ enabled: boolean, onNavigationBlocked: (path: string) => void }} options
 */
export function useUnsavedChangesGuard({ enabled, onNavigationBlocked }) {
